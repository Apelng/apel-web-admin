...
import { Plus, Pencil, Trash2, X, Check, ExternalLink, ToggleLeft, ToggleRight, Upload, ImageIcon } from 'lucide-react'
...
function PopupModal({ popup, onClose, onSaved }) {
  ...
  async function handleFile(file) {
    ...
    setUploading(true)
    try {
      const res = await uploadToCloudinary(file, 'apel-admin/apel-group/popups')
      set('imageUrl', res.secure_url)
    } catch {
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in">
      <div className="bg-white rounded-3xl shadow-modal w-full max-w-lg animate-scale-in">
        ...
        <div className="px-6 py-5 flex flex-col gap-4">
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1.5">
              Popup Image <span className="text-red-400">*</span>
            </label>
            ...
            {form.imageUrl && !uploading ? (
              ...
              <div className="relative rounded-xl overflow-hidden border border-slate-200 bg-slate-50 group">
                ...
                <div className="absolute inset-0 bg-black/0 group-hover:bg-black/30 transition-all flex items-center justify-center opacity-0 group-hover:opacity-100">
                  <button
                    ...
                    className="flex items-center gap-2 bg-white text-slate-700 text-xs font-semibold px-4 py-2 rounded-lg shadow-sm hover:bg-slate-50 transition-colors"
                  >
              ...
              <label
                ...
                className={clsx(
                  ...
                  : uploading ? 'border-brand-400 bg-brand-50/50'
                  : 'border-slate-200 hover:border-brand-400 hover:bg-slate-50'
                )}
              >
                <div className={clsx(
                  'w-12 h-12 rounded-2xl flex items-center justify-center',
                  uploading ? 'bg-brand-100' : 'bg-slate-100'
                )}>
                  {uploading
                    ? <svg className="animate-spin h-6 w-6 text-brand-600" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8H4z"/></svg>
            ...
            <div className="mt-2 flex items-center gap-2">
              ...
              <span className="text-[10px] text-slate-400 font-medium uppercase tracking-wide">or paste URL</span>
              <div className="h-px flex-1 bg-slate-100" />
            </div>
            <input
              value={form.imageUrl}
              ...
              className="mt-2 w-full rounded-xl border border-slate-200 px-3.5 py-2 text-xs text-slate-600 outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500"
            />
          </div>
        ...
        <div className="px-6 pb-5 flex justify-end gap-2.5 border-t border-slate-100 pt-4">
          ...
          <Button onClick={save} loading={saving} disabled={!form.imageUrl.trim() || uploading}>
